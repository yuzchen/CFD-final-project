function [rho, u, p, e] = Lax_Wendroff(dx, rho0, u0, p0, tEnd)
...
function L = Calculate_L_LW(U, dx, dt)
    global gamma CFL N epsilon
    rho = U(1,:);
    u = U(2,:)./rho;
    E = U(3,:)./rho;
    ...
    U_c = (U(:,1:N+2) + U(:,2:N+3)) / 2 - dt/dx/2 * (F(:,2:N+3) - F(:,1:N+2));  % U_{i+1/2}^{n+1/2}
    rho_c = U_c(1,:);
    u_c = U_c(2,:)./rho_c;
    E_c = U_c(3,:)./rho_c;
    p_c = (gamma-1).*(E_c-u_c.^2./2).*rho_c;
    H_c = (p_c * gamma) ./ ((gamma-1) * rho_c) + u_c.^2/2;
    F = [rho_c.*u_c; rho_c.*u_c.^2+p_c; rho_c.*u_c.*H_c]; % F(U_{i+1/2}^{n+1/2})
    % calculate L
    for i = 2:N+2
        L(:,i) = 1/dx * (F(:,i-1) - F(:,i));
    end
